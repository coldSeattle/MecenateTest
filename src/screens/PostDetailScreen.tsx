...
import { getPost, addComment } from '@/api/posts';
import { Avatar } from '@/components/ui/Avatar';
import { VerifiedBadge } from '@/components/ui/VerifiedBadge';
import { LikeButton } from '@/components/feed/LikeButton';
import { CommentBadge } from '@/components/feed/CommentBadge';
import { LockedBadge } from '@/components/feed/LockedBadge';
import { CommentItem } from '@/components/post/CommentItem';
import { CommentInput } from '@/components/post/CommentInput';
import { useComments, flattenCommentPages, commentsQueryKey } from '@/hooks/useComments';
...
export function PostDetailScreen({ postId }: PostDetailScreenProps) {
  ...
  const commentCount = commentsData?.pages[0]?.data
    ? comments.length
    : post.commentsCount;
          ...
          {isPaid ? (
            ...
            <>
              ...
              <YStack paddingHorizontal="$4" paddingTop="$3" gap="$2">
                <Text fontSize={20} fontWeight="800" color="$textPrimary" lineHeight={28}>
                  {post.title}
                </Text>
                <Text fontSize={15} color="$textSecondary" lineHeight={24}>
          ...
          <YStack style={{ backgroundColor: sectionBg }}>
            ...
            >
              ...
              <Pressable onPress={handleLoadNew} accessibilityRole="button" hitSlop={8}>
                <Text fontSize={14} fontWeight="600" style={{ color: '#7C3AED' }}>
                  Сначала новые
                </Text>
              </Pressable>
            </XStack>
            ...
            {commentsLoading ? (
              ...
              <YStack paddingVertical="$4" paddingHorizontal="$4">
                <Text fontSize={14} style={{ color: muteColor }}>
...
function Header({ onBack, borderColor, arrowColor }: HeaderProps) {
  return (
    <XStack
      ...
      alignItems="center"
      paddingHorizontal="$3"
      style={{ borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: borderColor }}
    >
